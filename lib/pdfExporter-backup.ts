...
export async function exportToPDF(
  ...
  const defaultOptions = {
    margin: [0.3, 0.4, 0.3, 0.4], // Top, Right, Bottom, Left - reduced margins
  ...
  try {
    ...
    loadingElement.innerHTML = `
      <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 9999; display: flex; align-items: center; justify-content: center; color: white; font-family: Poppins, sans-serif;">
    ...
    for (const mermaidEl of Array.from(mermaidElements)) {
      ...
      if (svgElement) {
        // Ensure mermaid diagrams are properly sized for PDF
